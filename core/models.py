...
class AuditLog(models.Model):
    ...
    def get_notification_text(self):
        # O resultado e renderizado com |safe no template, entao todo dado vindo
        # de usuario (nomes de aluno/curso/usuario) precisa ser escapado aqui.
        # As tags <strong> abaixo sao markup intencional e permanecem literais.
        if not self.content_type:
            nome = self.usuario.get_full_name() or self.usuario.username
            return f"{escape(nome)} realizou uma ação."

        model_name = self.content_type.model
        usuario_nome = self.usuario.get_full_name() or self.usuario.username if self.usuario else "Sistema"
        usuario_nome = escape(usuario_nome)

        try:
            if self.detalhes == "Qualitativo enviado para a Turma":
                obj = self.content_object
                curso_nome = escape(obj.nome if obj else "Desconhecido")
                return f"<strong>{usuario_nome}</strong> lançou qualitativos do curso <strong>{curso_nome}</strong>"

            if model_name == 'registroaula':
                obj = self.content_object
                curso_nome = escape(obj.curso.nome if obj and obj.curso else "Desconhecido")
                ...
                if obj and obj.inscricao:
                    aluno_nome = escape(obj.inscricao.aluno.nome_completo)
                    curso_nome = escape(obj.inscricao.curso.nome)
                    autor = escape(obj.professor_nome) if obj.professor_nome else usuario_nome
                    return f"<strong>{autor}</strong> avaliou o aluno <strong>{aluno_nome}</strong> no curso <strong>{curso_nome}</strong>"
                return f"<strong>{usuario_nome}</strong> preencheu uma avaliação de aluno"

            elif model_name == 'avaliacaoalunocurso':
                obj = self.content_object
                curso_nome = escape(obj.inscricao.curso.nome if obj and obj.inscricao else "Desconhecido")
                return f"<strong>{usuario_nome}</strong> preencheu a avaliação do curso <strong>{curso_nome}</strong>"

            elif model_name == 'arquivoaluno':
                obj = self.content_object
                aluno_nome = escape(obj.aluno.nome_completo if obj and obj.aluno else "Desconhecido")
                return f"<strong>{usuario_nome}</strong> atualizou um arquivo na ficha do aluno <strong>{aluno_nome}</strong>"

        except Exception:
            pass

        acao_str = escape(self.get_acao_display().lower())
        tabela = escape(self.content_type.name.title())
        nome_objeto = ""
        if self.content_object:
            nome_objeto = f" ({escape(str(self.content_object)[:30])})"
